...
const CountryDropdownComponent = (
	...
	useEffect(() => {
		if (defaultValue) {
			const initialCountry = options.find(country => country.countryCallingCodes[0] === defaultValue)
			if (initialCountry) {
							...
							{options
								...
								.map((option, key: number) => (
									<CommandItem className="flex items-center w-full gap-2" key={key} onSelect={() => handleSelect(option)}>
										...
										<CheckIcon
											className={cn('ml-auto h-4 w-4 shrink-0', option.name === selectedCountry?.name ? 'opacity-100' : 'opacity-0')}
										/>
									</CommandItem>
								))}
						</CommandGroup>
